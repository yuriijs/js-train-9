...
function getPositiveArrayValues(arr) {
  ...
  let positiveArrayValues = Array.from(arr.values()).filter(
    (element) => element > 0
  );
...
function removeAndDouble(arr, startIdx) {
  ...
  let doubledArray = arr.splice(0, startIdx).map((element) => element * 2);
  return doubledArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let sum = arr
    .slice(-n)
    .reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);
  arr.slice(-n).forEach((n) => {
    console.log(n);
  });
  return sum;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr) && typeof n === "number") {
    let expandedArray = arr.flatMap((number) => Array(n).fill(number));
